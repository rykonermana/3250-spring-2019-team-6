...
class TestClassFile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.constant_table_pre = {
            ...
            14: {'constant_code': 12, 'message': b'\x00\x06\x00\x07', 'decrypted_message': '<init>:()V'}, #How does <init> end up in quotes???
    ...
    def test_get_constant_pool_table(self):
        ...
        for i in self.constant_table_pre:
            self.assertEqual(test.constant_table[i]['constant_code'], self.constant_table_pre[i]['constant_code'])
            self.assertEqual(test.constant_table[i]['message'], self.constant_table_pre[i]['message'])

    def test_get_constant_messages(self):
        test = ConstantTable(self.data, self.count)
        for i in self.constant_table_pre:
            self.assertEqual(self.constant_table_pre[i]['decrypted_message'],test.constant_table[i]['decrypted_message'])